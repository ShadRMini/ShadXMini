...
import { Check, X as XIcon, Eye, RefreshCw } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <input
          ...
          className="ms-auto border border-slate-300 rounded-lg px-3 py-2 text-sm w-72 bg-white text-slate-900"
        />
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading && safeList.length === 0 ? (
                ...
                safeList.map((o) => (
                  <tr key={o.id || Math.random()} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="px-4 py-3 font-bold text-brand-600">#{o.orderNumber || o.id}</td>
                    <td className="px-4 py-3">{o.productName || "منتج"}</td>
                    ...
                    <td className="px-4 py-3 font-bold">${Number(o.totalUsd || 0).toFixed(2)}</td>
                    <td className="px-4 py-3">
                      <StatusPill s={o.status || "wait"} />
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-500">
                      {o.createdAt ? new Date(o.createdAt).toLocaleString("ar") : "—"}
                    </td>
                    <td className="px-4 py-3">
      ...
      {view && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-xl max-w-md w-full p-5 text-slate-900"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-bold">تفاصيل الطلب #{view.orderNumber || view.id}</h2>
            ...
            <dl className="space-y-2 text-sm">
              ...
              <Row k="الكمية" v={view.quantity ?? 1} />
              <Row k="المعرف" v={view.userIdentifier || "—"} />
              <Row
                k="الإجمالي"
                v={`$${Number(view.totalUsd || 0).toFixed(2)} / ${Number(view.totalSyp || 0).toFixed(0)} ل.س`}
              />
              <Row k="التكلفة" v={`$${Number(view.costUsd || 0).toFixed(2)}`} />
              <Row
                k="الربح"
                v={`$${(Number(view.totalUsd || 0) - Number(view.costUsd || 0)).toFixed(2)}`}
              />
              <Row k="الحالة" v={<StatusPill s={view.status || "wait"} />} />
              <Row
                k="التاريخ"
                v={view.createdAt ? new Date(view.createdAt).toLocaleString("ar") : "—"}
              />
            </dl>
          </div>
        </div>
