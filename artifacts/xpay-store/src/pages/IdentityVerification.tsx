    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <label className="text-xs font-bold text-[var(--theme-text-primary)] flex items-center gap-1.5">
        ...
        {value ? (
          <div className="relative group border border-[var(--theme-primary)]/50 rounded-2xl overflow-hidden bg-[var(--theme-background)] h-44 flex items-center justify-center shadow-lg">
            <img src={value} alt={label} className="w-full h-full object-cover" />
            <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
              <button
                type="button"
                onClick={() => setPreviewModalImg({ src: value, title: label })}
                className="px-3.5 py-2 bg-[var(--theme-primary)] text-black text-xs font-bold rounded-xl flex items-center gap-1.5 hover:opacity-90 transition-colors shadow-md"
              >
          ...
          <div
            ...
            className="border-2 border-dashed border-[var(--theme-primary)]/35 hover:border-[var(--theme-primary)] bg-[var(--theme-input-bg)]/50 hover:bg-[var(--theme-input-bg)]/80 rounded-2xl p-5 text-center cursor-pointer transition-all flex flex-col items-center justify-center min-h-[140px] group shadow-inner"
          ...
          >
            ...
            />
            <div className="w-11 h-11 rounded-full bg-[var(--theme-primary)]/15 group-hover:bg-[var(--theme-primary)]/25 flex items-center justify-center mb-2.5 text-[var(--theme-primary)] transition-colors">
              <Upload className="w-5 h-5" />
            </div>
            <p className="text-xs font-bold text-[var(--theme-text-primary)] group-hover:text-[var(--theme-accent)] mb-1 transition-colors">
              اضغط هنا أو اسحب الصورة لإرفاقها
            </p>
            <p className="text-[11px] text-[var(--theme-text-muted)] max-w-xs">{sublabel}</p>
          </div>
        )}
      </div>
    );
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-6" dir="rtl">
        <Loader2 className="w-10 h-10 text-[var(--theme-primary)] animate-spin mb-3" />
        <p className="text-sm font-bold text-[var(--theme-text-muted)]">جاري تحميل بيانات التوثيق...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[var(--theme-background)] text-[var(--theme-text-primary)] p-4 sm:p-6 pb-28 animate-in fade-in duration-300" dir="rtl">
      <div className="max-w-2xl mx-auto space-y-6">

        {/* Page Header */}
        <div className="flex items-center gap-3.5 bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-3xl p-5 shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 left-0 w-36 h-36 bg-[var(--theme-primary)]/10 rounded-full blur-2xl pointer-events-none" />
          <div className="w-13 h-13 rounded-2xl bg-[var(--theme-primary)] text-black flex items-center justify-center shadow-lg shrink-0">
            <ShieldCheck className="w-7 h-7" />
          </div>
          <div>
            <h1 className="text-xl font-black text-[var(--theme-primary)]">توثيق الهوية</h1>
        ...
        {verification && !showForm && (
          <div className="space-y-6">
            ...
            {verification.status === "pending" && (
              <div className="bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-3xl p-6 shadow-2xl relative overflow-hidden space-y-5">
                <div className="flex items-center gap-3.5 border-b border-[var(--theme-border)] pb-4">
                  <div className="w-11 h-11 rounded-2xl bg-[var(--theme-primary)]/20 text-[var(--theme-primary)] flex items-center justify-center shrink-0">
                    <Clock className="w-6 h-6 animate-pulse" />
                  </div>
                  <div>
                    <span className="inline-block px-3 py-1 rounded-full text-[11px] font-black bg-[var(--theme-primary)]/20 text-[var(--theme-primary)] border border-[var(--theme-primary)]/35 mb-1">
                      قيد المراجعة والتدقيق
                    </span>
                    <h2 className="text-base font-bold text-[var(--theme-text-primary)]">طلب توثيق الهوية قيد المراجعة</h2>
                  </div>
                </div>
                ...
                <div className="space-y-2.5 text-xs text-[var(--theme-text-primary)] bg-[var(--theme-background)] p-4 rounded-2xl border border-[var(--theme-border)]">
                  <div className="flex justify-between border-b border-[var(--theme-border)] pb-2">
                    <span className="text-[var(--theme-text-muted)]">الاسم الكامل:</span>
                    <span className="font-bold text-[var(--theme-accent)]">{verification.full_name}</span>
                  </div>
                  <div className="flex justify-between pt-0.5">
                    <span className="text-[var(--theme-text-muted)]">تاريخ تقديم الطلب:</span>
                ...
                <div className="space-y-2.5">
                  <span className="text-xs font-bold text-[var(--theme-primary)]">الصور المرفقة بالطلب:</span>
                  <div className="grid grid-cols-3 gap-2.5">
                    <div
                      onClick={() => setPreviewModalImg({ src: verification.id_front_image, title: "الوجه الأمامي" })}
                      className="cursor-pointer group relative border border-[var(--theme-border)] hover:border-[var(--theme-primary)] rounded-2xl overflow-hidden bg-black/40 h-24 transition-all"
                    >
                ...
                <div className="bg-[var(--theme-input-bg)]/40 border border-[var(--theme-border)] rounded-2xl p-3.5 text-xs text-[var(--theme-text-primary)] leading-relaxed flex items-start gap-2.5">
                  <Info className="w-4 h-4 text-[var(--theme-primary)] shrink-0 mt-0.5" />
                  <span>
            ...
            {verification.status === "rejected" && (
              <div className="bg-[var(--theme-card)] border border-rose-500/50 rounded-3xl p-6 shadow-2xl relative overflow-hidden space-y-5">
                <div className="flex items-center gap-3.5 border-b border-rose-500/20 pb-4">
                  ...
                  <div>
                    ...
                    <h2 className="text-base font-bold text-[var(--theme-text-primary)]">لم يتم قبول طلب التوثيق</h2>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 bg-[var(--theme-primary)] hover:opacity-90 text-black font-black text-xs rounded-2xl shadow-xl flex items-center justify-center gap-2 transition-all active:scale-[0.99]"
                >
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-3xl p-5 sm:p-7 shadow-2xl space-y-6">
            <div className="flex items-center justify-between border-b border-[var(--theme-border)] pb-4">
              <div>
                <h2 className="text-base font-bold text-[var(--theme-primary)]">نموذج توثيق الهوية</h2>
                <p className="text-xs text-[var(--theme-text-muted)]">يرجى تعبئة البيانات بدقة وإرفاق صور واضحة لوثائقك.</p>
              </div>
              {verification && verification.status === "rejected" && (
                <button
                  type="button"
                  onClick={() => setShowForm(false)}
                  className="text-xs text-[var(--theme-text-muted)] hover:text-[var(--theme-text-primary)] underline"
                >
                  إلغاء والعودة
                </button>
              )}
            ...
            {/* Instruction Banner */}
            <div className="bg-[var(--theme-background)] border border-[var(--theme-border)] rounded-2xl p-4 text-xs text-[var(--theme-text-primary)] space-y-2">
              <div className="font-bold text-[var(--theme-primary)] flex items-center gap-1.5">
                <Info className="w-4 h-4 text-[var(--theme-primary)]" />
                شروط قبول وثائق الهوية:
              </div>
              <ul className="list-disc list-inside space-y-1 text-[var(--theme-text-muted)] text-[11px] leading-relaxed">
            ...
            {/* Field 1: Full Name */}
            <div className="space-y-2">
              <label className="block text-xs font-bold text-[var(--theme-text-primary)] flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--theme-primary)]" />
              ...
              <input
                ...
                required
                className="w-full bg-[var(--theme-input-bg)] border border-[var(--theme-border)] focus:border-[var(--theme-primary)] focus:ring-1 focus:ring-[var(--theme-primary)] rounded-2xl px-4 py-3.5 text-xs text-[var(--theme-text-primary)] placeholder-gray-400 focus:outline-none transition-all shadow-inner"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-4 bg-[var(--theme-primary)] hover:opacity-90 text-black font-black text-sm rounded-2xl shadow-xl flex items-center justify-center gap-2 transition-all active:scale-[0.99] disabled:opacity-50 cursor-pointer"
            >
              {submitting ? (
                <>
      ...
      {previewModalImg && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="relative max-w-3xl w-full bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-3xl overflow-hidden p-3 shadow-2xl">
            <div className="flex items-center justify-between p-3 border-b border-[var(--theme-border)]">
              <span className="text-xs font-bold text-[var(--theme-accent)]">{previewModalImg.title}</span>
              <button
